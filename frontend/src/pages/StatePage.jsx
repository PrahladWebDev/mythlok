...
const StatePage = () => {
  ...
  const name = decodeURIComponent(stateName);
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    api.get(`/states/${encodeURIComponent(name)}`).then(r => setData(r.data.data)).catch(() => {}).finally(() => setLoading(false));
  }, [name]);
      ...
      <div className="state-page__hero">
        <div className="container">
          <nav className="state-page__breadcrumb">
            <Link to="/">Home</Link> / <Link to="/map">India Map</Link> / <span>{name}</span>
          </nav>
          ...
          <div className="state-page__actions">
            <Link to={`/explore?state=${encodeURIComponent(name)}`} className="btn btn-gold">Browse All Stories</Link>
        ...
        {data.stories?.length > 0 && (
          <section className="section">
            <div className="section-header">
              ...
              <Link to={`/explore?state=${encodeURIComponent(name)}`} className="btn btn-ghost btn-sm">View All →</Link>
            </div>
